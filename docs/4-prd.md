...
| 버전 | 날짜 | 작성자 | 변경 내용 |
|------|------|--------|-----------|
| 1.2.0 | 2026-05-15 | youngnam.her | BUG-001(비밀번호 401→400) 수정; Supabase 마이그레이션; Vercel 배포 완료; 완료된 할일 수정 불가 기능 추가; due_date 형식 버그 수정 |
| 1.1.0 | 2026-05-15 | youngnam.her | 다크모드·다국어(i18n) 구현 완료로 제외 범위에서 이동; 카테고리 다국어 이름(name_ko/name_en/name_zh) 도입; API 및 DB 스키마 반영 |
...
|------|------|---------|
| 호스팅 | Vercel | ✅ 완료 |
| FE 배포 | Vercel (정적 빌드) | ✅ https://ynher-fe.vercel.app |
| BE 배포 | Vercel (Node.js 서버) | ✅ https://ynher-be.vercel.app |
| DB | Supabase PostgreSQL (ap-northeast-2) | ✅ aws-1-ap-northeast-2.pooler.supabase.com:6543 |
| TLS | Vercel 제공 (자동) | ✅ HTTPS 적용 |
| CORS | FE 배포 URL 허용 | ✅ https://ynher-fe.vercel.app |
| 환경 변수 | Vercel Environment 관리 | ✅ 설정 완료 |
...
| 필드 | 타입 | 필수 | 유효성 규칙 |
|------|------|------|-------------|
| name_ko | String | Y | 최대 50자, 동일 사용자 내 중복 불가 (한국어 이름) |
| name_en | String | N | 최대 50자 (영어 이름, 선택) |
| name_zh | String | N | 최대 50자 (중국어 이름, 선택) |

**출력**

- 성공: HTTP 201, 생성된 카테고리 정보 (category_id, name_ko, name_en, name_zh, is_default, created_at)
...
| 항목 | 내용 |
|------|------|
| 기능 설명 | 인증된 사용자가 자신 소유의 할일에서 제목, 설명, 종료예정일, 카테고리를 수정한다. **완료된 할일(is_completed=true)은 수정 불가.** |
| 액터 | 인증 사용자 |
| 우선순위 | Must Have (MVP 1차) |
...
- 실패: HTTP 400 (제목 초과, 날짜 과거, 완료된 할일 수정 시도), HTTP 404 (타인 소유 또는 존재하지 않는 할일)
...
| BR-07 | 종료예정일이 KST 기준 오늘보다 이전이면 HTTP 400 |
| BR-11 | **완료된 할일(is_completed=true) 수정 불가; HTTP 400 TODO_ALREADY_COMPLETED** |
...
| UI 테마 | ✅ 라이트/다크 모드 (사용자 토글) | OAuth Social (Google, Facebook) |
| 다국어 | ✅ 한국어·영어·중국어 (i18n) | 추가 언어 확장 |
...
// 200 OK
{
  "success": true,
  "data": [
    { "category_id": "uuid-1", "name_ko": "일반", "name_en": "General", "name_zh": "一般", "is_default": true, "created_at": "2026-05-13T10:00:00.000Z" },
    { "category_id": "uuid-2", "name_ko": "업무", "name_en": "Work",    "name_zh": "工作", "is_default": true, "created_at": "2026-05-13T10:00:00.000Z" },
    { "category_id": "uuid-3", "name_ko": "개인", "name_en": "Personal","name_zh": "个人", "is_default": true, "created_at": "2026-05-13T10:00:00.000Z" },
    { "category_id": "uuid-4", "name_ko": "사이드 프로젝트", "name_en": "Side Project", "name_zh": null, "is_default": false, "created_at": "2026-05-13T12:00:00.000Z" }
  ]
}
```
...
| 필드 | 타입 | 필수 | 유효성 규칙 |
|------|------|------|-------------|
| name_ko | string | Y | 최대 50자, 동일 사용자(user_id) 내 중복 불가 |
| name_en | string | N | 최대 50자 |
| name_zh | string | N | 최대 50자 |

```json
// Request
{ "name_ko": "사이드 프로젝트", "name_en": "Side Project", "name_zh": "副业项目" }
```
...
// 400 — 완료된 할일 수정 시도
{ "success": false, "error": { "code": "TODO_ALREADY_COMPLETED", "message": "완료된 할일은 수정할 수 없습니다." } }
...
| name_ko | VARCHAR(50) | NOT NULL | 카테고리 이름 (한국어, 필수) |
| name_en | VARCHAR(50) | NULL | 카테고리 이름 (영어, 선택) |
| name_zh | VARCHAR(50) | NULL | 카테고리 이름 (중국어, 선택) |
| is_default | BOOLEAN | NOT NULL | 기본 카테고리 여부 |
| created_at | TIMESTAMPTZ | NOT NULL, DEFAULT NOW() | 생성 일시 |

- UNIQUE 제약: (user_id, name_ko) 복합 유니크 인덱스로 동일 사용자 내 한국어 카테고리명 중복 방지
- 기본 카테고리(일반·업무·개인)는 name_en, name_zh도 사전 정의된 번역값으로 채워짐
...
| categories | (user_id, name_ko) | UNIQUE INDEX | 한국어 카테고리명 중복 방지 |
...
### 8.5 구현 완료 항목 (기존 2차 계획 → 완료)

| 항목 | 구현 내용 |
|------|-----------|
| 다크 모드 | 헤더 토글 버튼으로 라이트/다크 전환; `localStorage`에 저장하여 새로고침 후에도 유지 |
| 다국어 지원 (i18n) | 한국어·영어·중국어 지원; 헤더 언어 선택 드롭다운; 카테고리 이름 언어별 DB 저장 |
...
| **BR-11** | **완료된 할일(is_completed=true) 수정 불가; HTTP 400** | **UC-09** |
